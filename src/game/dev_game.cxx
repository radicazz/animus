...
void game_create(engine::game_engine* engine) {
    ...
    state.player_entity = entities.create_interpolated_sprite({300, 300}, std::move(player_sprite));
    entities.add<engine::component_velocity>(state.player_entity) = {.linear = {0.0f, 0.0f},
                                                                     .angular = 0.f};

    // Asteroid
    auto asteroid_sprite =
        resource_manager.sprite_create("assets/sprites/asteroids/ice_1.png", {64, 64});
    state.asteroid_entity =
        entities.create_interpolated_sprite({300, 300}, std::move(asteroid_sprite));
    entities.add<engine::component_velocity>(state.asteroid_entity) = {.linear = {0.0f, 0.0f},
                                                                       .angular = 90.f};
...
void game_update(engine::game_engine* engine, float delta_time) {
    ...
    if (state.is_camera_free_mode == true) {
        ...
        const auto target = entities.get_interpolated_position(state.player_entity,
                                                               engine->get_interpolation_alpha());

        // Make the camera follow the player's interpolated position.
    ...
    if (input.is_key_pressed(engine::input_key::mouse_left) == true) {
        ...
        entities.set_position(state.asteroid_entity, world_pos);
    }
...
void game_render(engine::game_engine* engine, float interpolation_alpha) {
    ...
    if (entities.valid(state.player_entity) == true) {
        glm::vec2 player_position =
            entities.get_interpolated_position(state.player_entity, interpolation_alpha);
